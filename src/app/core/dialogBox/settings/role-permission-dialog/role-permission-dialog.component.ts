...
export class RolePermissionDialogComponent implements OnInit, AfterViewInit {
  ...
  initialReconnectionForm() {
    this.formGroup = this._formBuilder.group({
      webViewConnection: this.ifData
        ? this.UserPermisionArray.webViewConnection
        : true,
      webCreateTicketAndRequest: this.ifData
        ? this.UserPermisionArray.webCreateTicketAndRequest
        : '',
      webAdjustPayment: this.ifData
        ? this.UserPermisionArray.webAdjustPayment
        : '',
      webWareHouse: this.ifData ? this.UserPermisionArray.webWareHouse : '',

      webAddConnection: this.ifData
        ? this.UserPermisionArray.webAddConnection
        : '',
      webUpdateTicketAndRequest: this.ifData
        ? this.UserPermisionArray.webUpdateTicketAndRequest
        : '',
      webUserReport: this.ifData ? this.UserPermisionArray.webUserReport : '',
      webStore: this.ifData ? this.UserPermisionArray.webStore : '',

      webAlterConnection: this.ifData
        ? this.UserPermisionArray.webAlterConnection
        : '',
  ...
  putMethod(sendObj: any) {
    this.dataServise
      .putValue(`rolepermission/${this.UserPermisionArray.id}`, sendObj)
      .subscribe(
        (res: any) => {
          if (res.errorMessage) {
            this.loading = false;
