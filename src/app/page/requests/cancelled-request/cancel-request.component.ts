...
export class CancelledRequestComponent implements AfterViewInit, OnInit {
  ...
  getPendingData() {
    this.dataServise.getData(`request/status/cancelled`).subscribe(
      (res) => {
  ...
  exportNormalTable() {
    ...
    TableUtil.exportArrayToExcel(onlyNameAndSymbolArr, 'cancel-request');
  }
  ...
  public openPDF(): void {
    ...
    let doc = new jsPDF();
    doc.text('Cancel Requests/' + output + '/' + output2, 60, 10);
    autoTable(doc, {
      html: '#ExampleNormalTable',
      theme: 'striped',
      margin: { top: 20 },
    });
    doc.save('cancel-request ' + output + ' ' + output2);
  }
